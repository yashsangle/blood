...
#class RegisterForm(Form):
 #   name = StringField(
  #      'Name', [validators.DataRequired(), validators.Length(min=1, max=25)])
   # email = StringField(
    #    'Email', [validators.DataRequired(), validators.Length(min=10, max=50)])
...
#def register():
  #  form = RegisterForm(request.form)
    ...
    #     # Create cursor
    #     cur = mysql.connection.cursor()
    #     cur.execute("INSERT INTO RECEPTION(E_ID,NAME,EMAIL,PASSWORD) VALUES(%s, %s, %s, %s)",
    #                 (e_id, name, email, password))
    #     # Commit to DB
    #     mysql.connection.commit()
    #     # close connection
    #     cur.close()
...
#def bloodform():
    ...
    #     # Inserting values into tables
    #     cur.execute("INSERT INTO BLOOD(D_ID,B_GROUP,PACKETS) VALUES(%s, %s, %s)",
    #                 (d_id, blood_group, packets))
    #     cur.execute("SELECT * FROM BLOODBANK")
    #     records = cur.fetchall()
    #     cur.execute(
    #         "UPDATE BLOODBANK SET TOTAL_PACKETS = TOTAL_PACKETS+%s WHERE B_GROUP = %s", (packets, blood_group))
    #     # Commit to DB
    #     mysql.connection.commit()
    #     # close connection
    #     cur.close()
